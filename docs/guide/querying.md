...
When querying, the table name of the bundle is "data".
...
=== "Async API"
    ...
    ```python
    rs = await c.select("select * from data where age >= $1", [min_age])
    print(rs.to_polars())
    ```
...
=== "Sync API"
    ...
    ```python
    rs = c.select("select * from data where age >= $1", [min_age])
    print(rs.to_polars())
    ```
