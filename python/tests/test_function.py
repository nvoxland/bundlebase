...
Tests verify create_function, create_temporary_function, and drop_function
...
@pytest.mark.asyncio
async def test_create_function_persistent_ipc():
    """Test that create_function with ipc runner records a persistent operation."""
    with tempfile.TemporaryDirectory() as path:
        ...
        assert any(
            "CREATE FUNCTION" in change.description for change in status.changes
        )
        ...
        # Extend so we can inspect operations, then verify createFunction is present
        builder = await reopened.extend()
        ops = builder.operations()
        assert any(op.op_type == "createFunction" for op in ops)
...
async def test_function_name_validation():
    ...
    with pytest.raises((ValueError, Exception)):
        await c.create_function("double_val", ["Int64"], "Int64", "ipc", "./my_func")
...
@pytest.mark.asyncio
async def test_create_function_rejects_python_runner():
    """Test that persistent create_function rejects the python runner."""
    c = await bundlebase.create(random_bundle(), config=ALLOW_EXTERNAL_CODE_CONFIG)
...
@pytest.mark.asyncio
async def test_create_temporary_function():
    """Test that create_temporary_function does not record a persistent operation."""
    c = await bundlebase.create(random_bundle(), config=ALLOW_EXTERNAL_CODE_CONFIG)
    c = await c.create_temporary_function(
        "acme.double_val", ["Int64"], "Int64", "python", "test_function:double_val"
    )
    assert c is not None
    ...
    status = c.status()
    has_create_function = any(
        "CREATE FUNCTION" in change.description for change in status.changes
    )
    assert not has_create_function, (
...
async def test_drop_function():
    ...
    assert any("CREATE FUNCTION" in d for d in descriptions)
    assert any("DROP FUNCTION" in d for d in descriptions)


@pytest.mark.asyncio
async def test_create_function_with_platform():
    """Test CREATE FUNCTION with explicit platform."""
    c = await bundlebase.create(random_bundle(), config=ALLOW_EXTERNAL_CODE_CONFIG)
    c = await c.create_function(
...
@pytest.mark.asyncio
async def test_create_function_multiple_input_types():
    """Test CREATE FUNCTION with multiple input types."""
    c = await bundlebase.create(random_bundle(), config=ALLOW_EXTERNAL_CODE_CONFIG)
    c = await c.create_function(
...
@pytest.mark.asyncio
async def test_create_aggregate_function_via_api():
