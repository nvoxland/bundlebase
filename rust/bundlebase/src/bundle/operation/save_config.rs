...
mod tests {
    ...
    async fn test_check_rejects_secure_key() {
        ...
        let op = SaveConfigOp::setup("secret_access_key", "SECRETVALUE", &Scope::from_path("s3://bucket/").unwrap());
        ...
        let op = SaveConfigOp::setup("region", "us-west-2", &Scope::from_path("s3://bucket/").unwrap());
        let result = op.check(bundle.bundle()).await;
        assert!(result.is_ok());
    }
