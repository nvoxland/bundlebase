...
const MAGIC: &[u8; 8] = b"BBDELETE";
const VERSION: u16 = 1;
const HEADER_SIZE: usize = 8 + 2 + 8; // magic + version + count
...
pub fn deserialize_tombstone(bytes: &[u8]) -> Result<HashSet<RowId>, BundlebaseError> {
    ...
    let version = u16::from_le_bytes(
        bytes[8..10]
            .try_into()
            .map_err(|_| BundlebaseError::from("Invalid tombstone file: bad version"))?,
    );
    if version != VERSION {
        return Err(format!("Unsupported tombstone version: {}", version).into());
    }
...
mod tests {
    ...
    fn test_invalid_magic() {
        let bytes = b"WRONGMAG\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00";
    ...
    fn test_sorted_output() {
        ...
        let id1 = u64::from_le_bytes(bytes[18..26].try_into().expect("bad slice"));
        let id2 = u64::from_le_bytes(bytes[26..34].try_into().expect("bad slice"));
        let id3 = u64::from_le_bytes(bytes[34..42].try_into().expect("bad slice"));
        assert!(id1 < id2);
        assert!(id2 < id3);
    }
}
