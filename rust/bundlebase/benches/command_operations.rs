                        ...
                        std::thread::scope(|s| {
                            ...
                            s.spawn(move || {
                                ...
                                let data_path = url::Url::parse(&data_url)
                                    ...
                                    .to_file_path()
                                    .expect("to file path");
                                let dest_file = source_dir.join(
                                    data_path.file_name().expect("file name"),
                                );
                                std::fs::copy(&data_path, &dest_file)
                                    .expect("copy data file");
                                ...
                                let bundle = setup_rt
                                    .block_on(async {
                                        let bundle =
                                            BundleBuilder::create(&bundle_url, None).await?;
                                        ...
                                        bundle
                                            .create_source("remote_dir", args, None)
                                            .await?;
