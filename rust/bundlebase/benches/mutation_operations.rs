        ...
        group.bench_with_input(
            ...
            |b, &rows| {
                b.to_async(&rt).iter_batched(
                    || setup_bundle(&rt, rows, &format),
                    |bundle| async move {
                        bundle.delete("filter_value < 10").await.expect("delete failed");
    ...
    for rows in [SCALE_1K, SCALE_10K, SCALE_100K] {
        ...
        // Query after 10% delete (committed + reopened)
        {
            let bundle = rt.block_on(async {
                let b = create_benchmark_bundle(rows, &format).await.expect("bundle");
            ...
            |b, &rows| {
                b.to_async(&rt).iter_batched(
                    ...
                    |bundle| async move {
                        ...
                        bundle.delete("filter_value < 5").await.expect("delete failed");
                        bundle
                            .update("SET amount = amount * 2 WHERE filter_value >= 5 AND filter_value < 15")
                            .await
                            .expect("update failed");
                        bundle.commit("Delete + Update").await.expect("commit failed");
                    },
                    criterion::BatchSize::SmallInput,
                );
            },
