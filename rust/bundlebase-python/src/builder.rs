...
impl PyBundleBuilder {
    ...
    fn save_config<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            inner
                .save_config(&scope, key.as_str(), value.as_str())
                .await
    ...
    fn normalize_column_names<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            inner
                ...
                .await
                .map_err(|e| to_py_error_ctx("Failed to normalize column names", e))?;
            Python::attach(|py| Py::new(py, PyBundleBuilder { inner }).map_err(|e| to_py_error(e)))
        })
    }
    ...
    fn fetch<'py>(
        ...
        let sync_mode =
            SyncMode::from_arg(mode).map_err(|e| to_py_error_ctx("Invalid sync mode", e))?;
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let results = inner
                .fetch(&pack, sync_mode)
                .await
                .map_err(|e| to_py_error_ctx("Failed to fetch", e))?;
            let py_results: Vec<PyFetchResults> =
                results.iter().map(PyFetchResults::from_rust).collect();
            Ok(py_results)
        })
    }
    ...
    fn extend<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let new_builder = inner
                .extend(data_dir_owned.as_deref())
                .await
    ...
    fn commit<'py>(&self, message: &str, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            inner
                .commit(&message)
                .await
                .map_err(|e| to_py_error_ctx("Failed to commit", e))?;
            Ok(())
        })
    }
    ...
    fn describe_data<'py>(
        ...
        // Parse columns: accept strings or (name, type) tuples
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let stream = inner
                ...
                .await
                .map_err(|e| to_py_error_ctx("Failed to describe data", e))?;
            let schema = std::sync::Arc::new(stream.schema().as_ref().clone());
            Python::attach(|py| {
                Py::new(
                    py,
    ...
    fn test_connector<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let stream = inner
                ...
                .await
                .map_err(|e| to_py_error_ctx("Failed to test connector", e))?;
            let schema = std::sync::Arc::new(stream.schema().as_ref().clone());
            Python::attach(|py| {
                Py::new(
                    py,
    ...
    fn test_temp_connector<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let stream = inner
                ...
                .await
                .map_err(|e| to_py_error_ctx("Failed to test temp connector", e))?;
            let schema = std::sync::Arc::new(stream.schema().as_ref().clone());
            Python::attach(|py| {
                Py::new(
                    py,
    ...
    fn rebuild_index<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            inner
                ...
                .await
                .map_err(|e| to_py_error_ctx("Failed to rebuild index", e))?;
            Python::attach(|py| Py::new(py, PyBundleBuilder { inner }).map_err(|e| to_py_error(e)))
        })
    }
    ...
    fn drop_index<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            inner
                .drop_index(&column)
                .await
                .map_err(|e| to_py_error_ctx("Failed to drop index", e))?;
            Python::attach(|py| Py::new(py, PyBundleBuilder { inner }).map_err(|e| to_py_error(e)))
        })
    }
    ...
    fn view<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            ...
            Python::attach(|py| {
                Py::new(py, super::bundle::PyBundle::new(bundle)).map_err(|e| to_py_error(e))
            })
        })
    }
    ...
    fn verify_data<'py>(
        ...
        let inner = self.inner.clone();
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let results = inner.verify_data(update_versions).await.map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            ...
            Python::attach(|py| {
                Py::new(py, super::bundle::PyVerificationResults::from(&results)).map_err(|e| {
