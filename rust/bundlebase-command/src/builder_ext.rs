...
use crate::builder::{
    AddColumnCommand, AlwaysDeleteCommand, AttachCommand, CastColumnCommand, CreateIndexCommand,
...
pub trait BundleBuilderExt {
    ...
    async fn fetch(&self, pack: &str, mode: SyncMode)
    ...
    /// Rename a column.
    async fn rename_column(&self, old_name: &str, new_name: &str)
        -> Result<&Self, BundlebaseError>;
    ...
    async fn filter(&self, query: &str, params: Vec<ScalarValue>)
        -> Result<&Self, BundlebaseError>;
...
impl BundleBuilderExt for BundleBuilder {
    async fn attach(&self, path: &str, pack: Option<&str>) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, AttachCommand::new(path, pack.map(|s| s.to_string()))).await?;
        Ok(self)
    }

    async fn detach_block(&self, location: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, DetachBlockCommand::new(location)).await?;
        Ok(self)
    }
    ...
    async fn replace_block(
        ...
        exec_cmd(self, ReplaceBlockCommand::new(old_location, new_location)).await?;
        Ok(self)
    }
    ...
    async fn rename_connector(
        ...
        exec_cmd(self, RenameConnectorCommand::new(old_name, new_name)).await?;
        Ok(self)
    }
    ...
    async fn drop_connector(
        ...
        exec_cmd(self, DropConnectorCommand::new(name, platform)).await?;
        Ok(self)
    }
    ...
    async fn rename_view(&self, old_name: &str, new_name: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, RenameViewCommand::new(old_name, new_name)).await?;
        Ok(self)
    }

    async fn drop_view(&self, view_name: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, DropViewCommand::new(view_name)).await?;
        Ok(self)
    }
    ...
    async fn create_report(
        ...
        exec_cmd(
            self,
            CreateReportCommand::new(id, name, description, content),
        )
        .await?;
        Ok(self)
    }
    ...
    async fn drop_report(&self, id: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, DropReportCommand::new(id)).await?;
        Ok(self)
    }

    async fn drop_join(&self, join_name: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, DropJoinCommand::new(join_name)).await?;
        Ok(self)
    }

    async fn rename_join(&self, old_name: &str, new_name: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, RenameJoinCommand::new(old_name, new_name)).await?;
        Ok(self)
    }

    async fn drop_column(&self, name: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, DropColumnCommand::new(name)).await?;
        Ok(self)
    }
    ...
    async fn rename_column(
        ...
        exec_cmd(self, RenameColumnCommand::new(old_name, new_name)).await?;
        Ok(self)
    }

    async fn add_column(&self, name: &str, expression: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, AddColumnCommand::new(name, expression)).await?;
        Ok(self)
    }

    async fn cast_column(&self, name: &str, new_type: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, CastColumnCommand::new(name, new_type)).await?;
        Ok(self)
    }

    async fn normalize_column_names(&self) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, NormalizeColumnNamesCommand).await?;
        Ok(self)
    }
    ...
    async fn filter(
        ...
        exec_cmd(self, FilterCommand::new(query, params)).await?;
        Ok(self)
    }
    ...
    async fn drop_always_delete(
        ...
        exec_cmd(
            self,
            DropAlwaysDeleteCommand::new(where_clause.map(|s| s.to_string())),
        )
        .await?;
        Ok(self)
    }
    ...
    async fn always_update(&self, set_where: &str) -> Result<&Self, BundlebaseError> {
        ...
        let cmd = crate::parser::parse_command(&full_sql)?;
        match cmd {
            crate::BundleCommand::AlwaysUpdate(cmd) => {
    ...
    async fn drop_always_update(&self, rule_text: Option<&str>) -> Result<&Self, BundlebaseError> {
        exec_cmd(
            self,
            DropAlwaysUpdateCommand::new(rule_text.map(|s| s.to_string())),
        )
        .await?;
        Ok(self)
    }
    ...
    async fn join(
        ...
        exec_cmd(
            self,
            JoinCommand::new(name, expression, location.map(|s| s.to_string()), join_type),
        )
        .await?;
        Ok(self)
    }
    ...
    async fn import_function(
        ...
        let platform: Platform = platform.parse()?;
        exec_cmd(self, ImportFunctionCommand::new(name, from, platform)).await?;
        Ok(self)
    }
    ...
    async fn rename_function(
        ...
        exec_cmd(self, RenameFunctionCommand::new(old_name, new_name)).await?;
        Ok(self)
    }
    ...
    async fn drop_function(
        ...
        let platform: Option<Platform> = platform.map(|s| s.parse()).transpose()?;
        exec_cmd(self, DropFunctionCommand::new(name, platform)).await?;
        Ok(self)
    }

    async fn set_name(&self, name: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, SetNameCommand::new(name)).await?;
        Ok(self)
    }

    async fn set_description(&self, description: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, SetDescriptionCommand::new(description)).await?;
        Ok(self)
    }
    ...
    async fn save_config(
        ...
        exec_cmd(self, SaveConfigCommand::new(scope.clone(), key, value)).await?;
        Ok(self)
    }
    ...
    async fn create_index(
        ...
        exec_cmd(
            self,
            CreateIndexCommand::new(cols, index_type, name.map(|s| s.to_string())),
        )
        .await?;
        Ok(self)
    }
    ...
    async fn drop_index(&self, column: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, DropIndexCommand::new(column)).await?;
        Ok(self)
    }

    async fn rebuild_index(&self, column: &str) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, RebuildIndexCommand::new(column)).await?;
        Ok(self)
    }

    async fn reindex(&self) -> Result<&Self, BundlebaseError> {
        exec_cmd(self, ReindexCommand::new()).await?;
        Ok(self)
    }
...
impl BundleBuilderExt for Arc<BundleBuilder> {
    ...
    async fn replace_block(
        ...
        (**self).replace_block(old_location, new_location).await?;
        Ok(self)
    }
    async fn create_source(
        &self,
    ...
    async fn rename_connector(
        ...
        (**self).rename_connector(old_name, new_name).await?;
        Ok(self)
    }
    async fn drop_connector(
        &self,
        ...
        (**self).drop_connector(name, platform).await?;
        Ok(self)
    }
    async fn fetch(
        &self,
    ...
    async fn rename_view(&self, old_name: &str, new_name: &str) -> Result<&Self, BundlebaseError> {
        (**self).rename_view(old_name, new_name).await?;
        Ok(self)
    }
    async fn drop_view(&self, view_name: &str) -> Result<&Self, BundlebaseError> {
        (**self).drop_view(view_name).await?;
        Ok(self)
    }
    async fn create_report(
        &self,
    ...
    async fn drop_report(&self, id: &str) -> Result<&Self, BundlebaseError> {
        (**self).drop_report(id).await?;
        Ok(self)
    }
    async fn drop_join(&self, join_name: &str) -> Result<&Self, BundlebaseError> {
        (**self).drop_join(join_name).await?;
        Ok(self)
    }
    async fn rename_join(&self, old_name: &str, new_name: &str) -> Result<&Self, BundlebaseError> {
        (**self).rename_join(old_name, new_name).await?;
        Ok(self)
    }
    async fn drop_column(&self, name: &str) -> Result<&Self, BundlebaseError> {
        (**self).drop_column(name).await?;
        Ok(self)
    }
    async fn rename_column(
        &self,
        ...
        (**self).rename_column(old_name, new_name).await?;
        Ok(self)
    }
    async fn add_column(&self, name: &str, expression: &str) -> Result<&Self, BundlebaseError> {
        (**self).add_column(name, expression).await?;
        Ok(self)
    }
    async fn cast_column(&self, name: &str, new_type: &str) -> Result<&Self, BundlebaseError> {
        (**self).cast_column(name, new_type).await?;
        Ok(self)
    }
    async fn normalize_column_names(&self) -> Result<&Self, BundlebaseError> {
        (**self).normalize_column_names().await?;
        Ok(self)
    }
    async fn filter(
        &self,
        ...
        (**self).filter(query, params).await?;
        Ok(self)
    }
    async fn delete(&self, where_clause: &str) -> Result<usize, BundlebaseError> {
    ...
    async fn update(&self, set_where: &str) -> Result<&Self, BundlebaseError> {
        (**self).update(set_where).await?;
        Ok(self)
    }
    async fn always_delete(&self, where_clause: &str) -> Result<&Self, BundlebaseError> {
        (**self).always_delete(where_clause).await?;
        Ok(self)
    }
    async fn drop_always_delete(
        &self,
        ...
        (**self).drop_always_delete(where_clause).await?;
        Ok(self)
    }
    async fn always_update(&self, set_where: &str) -> Result<&Self, BundlebaseError> {
        (**self).always_update(set_where).await?;
        Ok(self)
    }
    async fn drop_always_update(&self, rule_text: Option<&str>) -> Result<&Self, BundlebaseError> {
        (**self).drop_always_update(rule_text).await?;
        Ok(self)
    }
    async fn join(
        &self,
        ...
        (**self).join(name, expression, location, join_type).await?;
        Ok(self)
    }
    async fn import_function(
        &self,
        ...
        (**self).import_function(name, from, platform).await?;
        Ok(self)
    }
    async fn rename_function(
        &self,
        ...
        (**self).rename_function(old_name, new_name).await?;
        Ok(self)
    }
    async fn drop_function(
        &self,
        ...
        (**self).drop_function(name, platform).await?;
        Ok(self)
    }
    async fn set_name(&self, name: &str) -> Result<&Self, BundlebaseError> {
    ...
    async fn save_config(
        ...
        (**self).save_config(scope, key, value).await?;
        Ok(self)
    }
    async fn create_index(
        &self,
        ...
        (**self).create_index(columns, index_type, name).await?;
        Ok(self)
    }
    async fn drop_index(&self, column: &str) -> Result<&Self, BundlebaseError> {
        (**self).drop_index(column).await?;
        Ok(self)
    }
    async fn rebuild_index(&self, column: &str) -> Result<&Self, BundlebaseError> {
        (**self).rebuild_index(column).await?;
        Ok(self)
    }
    async fn reindex(&self) -> Result<&Self, BundlebaseError> {
        (**self).reindex().await?;
